...
# Plot
png(file=outfile1, width=2000, height=1000)
p1 <- DimPlot(seurat.integrated, reduction = "umap", group.by="sample", label=TRUE, pt.size=2, label.size=6) + NoLegend()
p2 <- DimPlot(seurat.integrated, reduction = "umap", label=FALSE, pt.size=2, label.size=6)
p1 + p2
dev.off()

png(file=outfile2, width=3000, height=1000)
DimPlot(seurat.integrated, reduction = "umap", split.by="sample",
    ncol=5, label=FALSE, pt.size=2, label.size=6)
dev.off()