...
# 集計：time × celltype ごとの割合
agg <- aggregate(is_basin ~ time + celltype, data = cell_state, FUN = mean)
colnames(agg)[3] <- "frac_basin"
...
# 角度計算
pie_df <- pie_df[order(pie_df$time, pie_df$celltype, pie_df$kind), ]
pie_df$start <- ave(pie_df$value, pie_df$time, pie_df$celltype,
...
ggsave(outfile, p, width = 8, height = 4, dpi = 300)