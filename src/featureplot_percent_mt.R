...
percent.mt <- colSums(seurat.obj[target.genes, ]) / colSums(seurat.obj) * 100
seurat.obj[["percent.mt"]] <- percent.mt

# Plot
png(file=outfile, width=600, height=600)
FeaturePlot(seurat.obj, features="percent.mt", pt.size=2, label.size=6)
dev.off()
