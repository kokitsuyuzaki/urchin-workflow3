...
for(i in seq_len(ncol(bindata_cont))){
     cmd <- paste0("seurat.cont$bindata_", i, " <- bindata_cont[, i]")
     eval(parse(text=cmd))
}
...
for(i in seq_len(ncol(bindata_cont))){
     ...
     # Plot
     g <- DimPlot(seurat.cont, reduction = "umap", group.by=groupname, label=TRUE, pt.size=2, label.size=6, cols=c(4,2)) + NoLegend()
     png(file=filename1, width=600, height=600)
     print(g)
     dev.off()
     # Plot
     g <- DimPlot(seurat.cont, reduction = "umap", group.by=groupname, split.by="sample",
